...
sentinel_cfg = DMSConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_dmsd_proto_version_with_sentinel_ping = 70207


def get_dms_conf():
    if sys.platform == 'win32':
        dms_conf = os.path.join(os.getenv('APPDATA'), "DMSCore/dms.conf")
    else:
        home = os.environ.get('HOME')

        dms_conf = os.path.join(home, ".dmscore/dms.conf")
        if sys.platform == 'darwin':
            dms_conf = os.path.join(home, "Library/Application Support/DMSCore/dms.conf")

    dms_conf = sentinel_cfg.get('dms_conf', dms_conf)
