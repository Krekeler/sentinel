...
def is_valid_dms_address(address, network='mainnet'):
    ...
    dms_version = 140 if network == 'testnet' else 76     
    ...
    try:
        ...
        # rescue from exception, not a valid DMS address
        return False

    if (address_version != dms_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('dms.conf')
