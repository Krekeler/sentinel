...
class DMSDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some dmsd methods
    ...
    @classmethod
    def from_dms_conf(self, dms_dot_conf):
        from dms_config import DMSConfig
        config_text = DMSConfig.slurp_config_file(dms_dot_conf)
        creds = DMSConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = dmslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = dmslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = dmslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_dmsd_proto_version_with_sentinel_ping)
