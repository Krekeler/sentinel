...
def has_dms_conf():
    ...
    valid_dms_conf = False

    # ensure dms_conf exists & readable
    #
    # if not, print a message stating that DMS Core must be installed and
    # configured, including JSONRPC access in dms.conf
    try:
        f = io.open(config.dms_conf)
        valid_dms_conf = True
    except IOError as e:
        print(e)

    return valid_dms_conf
...
def main():
    ...
    if not has_dms_conf():
        print("DMS Core must be installed and configured, including JSONRPC access in dms.conf")
        sys.exit(1)
