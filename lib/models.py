...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, dmsd):
        golist = dmsd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_dms(dmsd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_dms(self, dmsd, rec):
        import decimal
        import dmslib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from dmsd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from dmsd, with every run
        ...
        # get/create, then sync payment amounts, etc. from dmsd - dmsd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(dmsd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from dmsd! %s" % e)
            govobj.vote_delete(dmsd)
            return (govobj, None)
    ...
    def vote_delete(self, dmsd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(dmsd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, dmsd, signal, outcome):
        import dmslib
        ...
        output = dmsd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = dmslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(dmsd, signal)

    def sync_network_vote(self, dmsd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = dmsd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = DMSD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 DMS addr, non-multisig
            if not dmslib.is_valid_dms_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid DMS address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # DMS Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (dmslib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = DMSD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not dmslib.is_valid_dms_address(addr, config.network):
    ...
    def hash(self):
        import dmslib
        return dmslib.hashit(self.serialise())
