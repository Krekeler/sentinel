...
def test_valid_dms_address():
    from dmslib import is_valid_dms_address

    main = valid_dms_address()
    test = valid_dms_address('testnet')

    assert is_valid_dms_address(main) is True
    assert is_valid_dms_address(main, 'mainnet') is True
    assert is_valid_dms_address(main, 'testnet') is False

    assert is_valid_dms_address(test) is False
    assert is_valid_dms_address(test, 'mainnet') is False
    assert is_valid_dms_address(test, 'testnet') is True


def test_invalid_dms_address():
    from dmslib import is_valid_dms_address

    main = invalid_dms_address()
    test = invalid_dms_address('testnet')

    assert is_valid_dms_address(main) is False
    assert is_valid_dms_address(main, 'mainnet') is False
    assert is_valid_dms_address(main, 'testnet') is False

    assert is_valid_dms_address(test) is False
    assert is_valid_dms_address(test, 'mainnet') is False
    assert is_valid_dms_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from dmslib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % dmslib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(dmslib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(dmslib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(dmslib.blocks_to_seconds(16616)) == 2612035
